...
def processKey(observer, board, key):
	global nBlocks 

	state = board.accept(key)

	observer.screen = board.getScreen()
	observer.checkNum = True
...
class Model(threading.Thread, Observer, Publisher):
	...
	def run(self):
		...
		self.observer.screen = board.getScreen()
		self.observer.checkNum = True
		...
		while not isGameDone:
			...
			if key == 'q':
				...
				state = processKey(self.observer, board, key)
	...
	def run(self):
		while True:
			if self.checkNum == True:
				printWindow(self.window, self.screen)
				self.checkNum == False
